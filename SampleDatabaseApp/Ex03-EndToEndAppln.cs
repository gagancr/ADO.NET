using SampleDatabaseApp.Dalayer;
using SampleDatabaseApp.Entities;
using System;
...
namespace SampleDatabaseApp
{
    ...
    namespace Dalayer
    {
        ...
        class DataComponent : IDataAccessComponent
        {
            ...
            const string STRINSERT = "InsertEmployees";
            const string STRUPDATE = "Update employees Set EmpName = @empName, EmpAdress = @empAddress, EmpSalary = @empSalary WHERE EmpId = @empId";
            const string STRALL = "SELECT * FROM employees";
            const string STRALLDEPTS = "SELECT * FROM TBLDEPT";
            const string STRDELETE = "DELETE FROM EMPLOYEES WHERE EMPID = @id";
            #endregion
